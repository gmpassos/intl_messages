...
void main() {
  group('IntlRawEntry', () {
    ...
    test('checkIntlEntries[OK]', () async {
      var lines = ['msgA=aaa aaa', 'msgB=bbb bbb'];

      var entries = IntlRawEntry.fromLines(lines)!;

      var logs = <String>[];

      var entries2 = checkIntlEntries(
        entries,
    ...
    test('checkIntlEntries[OK: IGNORING IDENTICAL DUPLICATES]', () async {
      var lines = ['msgA=aaa aaa', 'msgB=bbb bbb', 'msgB=bbb bbb'];
    ...
    test('checkIntlEntries[ERROR: DUPLICATED]', () async {
      var lines = ['msgA=aaa aaa', 'msgB=bbb bbb', 'msgB=bbb bbb bbb'];
    ...
    test('checkIntlEntriesReference[OK]', () async {
      var linesRef = ['msgA=aaa aaa', '', 'msgB=bbb bbb'];

      var lines = ['msgB=BBB', 'msgA=AAA', 'msgB=BBB', 'msgA=AAA'];
      ...
      var errorKeys = checkIntlEntriesReference(
        entriesRef,
        entries,
        log: (t, m) => logs.add('$t>>$m'),
      );
    ...
    test('checkIntlEntriesReference[ERROR]', () async {
      ...
      var lines = ['msgB=BBB', 'msgX=XXX', 'msgB=BBB'];
      ...
      expect(
        logs,
        equals(['CHECK>>Missing keys: [msgA]', 'CHECK>>Extra keys: [msgX]']),
      );
    });
    ...
    test('checkIntlEntriesReference[OK]', () async {
      ...
      var lines = ['msgB=BBB', 'msgA=AAA', 'msgB=BBB'];
