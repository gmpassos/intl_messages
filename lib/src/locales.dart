...
class LocaleOption {
  ...
  @override
  String toString() {
    return 'LocaleOption{locale: $locale, selected: $selected, name: $name}';
  }
}
...
abstract class LocalesManager {
  static final List<LocalesManager> _instances = [];

  static List<LocalesManager> instances() => List.from(_instances);

  static List<LocalesManager> instancesInitialized() =>
      List<LocalesManager>.from(_instances)
        ..retainWhere((l) => l.getInitializedLocales().isNotEmpty);

  static final EventStream<String> onDefineLocaleLibraryIntegration =
      EventStream();
  ...
  List<String> getInitializedLocales() {
    return _initializedLocales.entries
        .where((e) => e.value)
        .map((e) => e.key)
        .toList();
  }
...
List<String> getPossibleLocalesSequence(String locale) {
  return getPossibleLocalesSequenceImpl(locale);
}
